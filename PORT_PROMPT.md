...
Use this document as the starting prompt when implementing a new language port of the universalWasmLoader spec. It captures the design ideal, the proven architecture, and the hard-won lessons from the Rust port.
...
| Resource | Location |
| --- | --- |
| Spec | `../universalWasmLoader-js/SPEC.md` |
| JS/TS reference impl | `../universalWasmLoader-js/` — read `abi.js`, `wit-parser.js`, `universal-wasm-loader.js` |
| Rust port (complete) | `../universalWasmLoader-rs/` — `src/lib.rs`, `src/abi.rs`, `src/wit_parser.rs` |
| Test fixtures | `../universalWasmLoader-js/tests/` — copy `.wasm` + `.wit` pairs, do not hand-edit binaries |
...
A language-idiomatic way to declare a named handle with typed methods. In Rust this is a `macro_rules!` macro; in Python a class decorator; in Go a code-generated struct; in C# a source generator or generic wrapper. The `load` constructor must be **synchronous**.

```
// The target ergonomics (Rust syntax shown):
wasm_interface! {
    pub struct Math {
...
math = Math.load("math.wasm")
result = math.add(3, 4)
```

### 5. `create_singleton(path, callbacks?) → Loader`

Returns a callable that loads once on first call and returns the same handle on every subsequent call. All callers share the same instance (reference-counted or equivalent). Provides an identity-check method (`ptr_eq` in Rust, `is` in Python, etc.).
...
| Binary name | camelCase | `isPositive` | Actual WASM export name — use this when calling `wasmtime`/runtime |
| API name | snake_case (or lang-idiomatic) | `is_positive` | User-facing method/key name |
...
Register callbacks in the WASM linker under the `"env"` namespace before instantiation. Memory (`env.memory`) is only available after instantiation, so **read memory from the caller context at call time**, not during registration. This avoids the chicken-and-egg problem.
...
This is the key lesson from the Rust port. The following must be handled **inside the library**, never delegated to the caller:
...
The test suite must pass all assertions from [SPEC.md §8](../universalWasmLoader-js/SPEC.md). Fixture files live in `tests/` — copy from `../universalWasmLoader-js/tests/`.
...
After implementing the base spec suite, add tests for the three user-facing patterns:

| Pattern | Tests |
| --- | --- |
| `wasm_load` + `call` | All spec fixtures without async |
| `bind` | Bind one export; bind multiple exports from the same handle |
| Typed interface | One interface per fixture covering all exports |

All tests should use the language's idiomatic error propagation — no manual unwrapping or try/catch at each assertion.

---

## Implementation Order

1. **WIT parser** — regex-based, no external dependencies. Parse `name`, `camel_name`, `snake_name` from `.wit` source. Verify against the JS reference.
2. **ABI layer** — implement `call_wasic_export`, `call_component_export`, `call_raw_export`, and `link_import_env`. Verify each with the corresponding fixture before moving on.
3. **Core handle** — the `ModuleExports`/equivalent type that wraps runtime state and exposes `call` / `call_dyn`.
4. **`wasm_load`** — synchronous file loader. Run the spec test suite against it before adding any async code.
5. **`wasm_import`** — async file + URL loader. Reuse the same instantiation path; only the IO layer differs.
6. **`bind`** — thin wrapper over `call`; one or two lines.
7. **Typed interface** — macro, decorator, or codegen pattern; calls `wasm_load` internally.
8. **`create_singleton`** — one-time init with language's lazy/once primitive.
...
- **Do not expose the WASM runtime type system** — users work with native language types only (`int`, `float`, `str`, `bool`). The port translates to/from WASM values internally.
- **Callbacks must clone/capture** — the host callback must own everything it needs; it will be called from inside the WASM runtime after the setup scope has ended.
- **Blocking work in async runtimes** — compilation (`Module::new` equivalent) is CPU-intensive. Offload it to a thread pool when called from an async context. Never call blocking HTTP inside a blocking-thread context — do the HTTP fetch in the async context first, then hand off bytes.
- **`once_cell` / lazy equivalent for singleton** — use the language's standard lazy-initialization primitive. In Rust, `std::sync::OnceLock::get_or_try_init` requires nightly; use `once_cell::sync::OnceCell` instead.
- **Do not use `snake_name` to look up WASM exports** — always use `camel_name`. This is the single most common source of "export not found" errors.
- **Pin the WASM runtime version** — ABI behavior changes between major versions. Document the pinned version in the project manifest.
